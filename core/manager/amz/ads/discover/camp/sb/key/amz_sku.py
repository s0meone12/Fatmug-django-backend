...
from core.models import SBDiscKeyTgt, AmzSku
from django.db.models import Count, F, Window
from .main import AmzSbDiscKeyCampManager


class AmzSkuSubManagerSbDiscKeyCamp(metaclass=CheckQSMeta):
    ...
    def no_of_live_targets(self, qs=None) -> pd.DataFrame:
        ...
        """
        qs = self.get_qs(qs=qs)
        df = SBDiscKeyTgt.manager.camp.no_of_live_targets(
    ...
    def no_of_targets_pending_campaign_addition(self, qs=None) -> int:
        ...
        """
        discovered_keyword_targets = SBDiscKeyTgt.manager.filter(
    ...
    def __generate_new_names(self, qs=None) -> list[str]:
        ...
        for qs_i in qs:
            ...
            for i in range(num_of_new_camps_required):
                ...
                name = f"{initial_letter_for_camp_name}^Vdo_{qs_i.int_sku.name}_{qs_i.asin}_Key"
