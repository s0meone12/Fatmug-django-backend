...
class SPDiscProTgtManager(BaseManager):

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.__amz_sku = None
        self.__camp = None
        self.__sku_disc_tgt = None
    ...
    def amz_sku(self):
        if not self.__amz_sku:
            from .amz_sku import AmzSkuSubManagerSPDiscProTgt
            self.__amz_sku = AmzSkuSubManagerSPDiscProTgt(self)
        return self.__amz_sku

    @property
    def sku_disc_tgt(self):
        if not self.__sku_disc_tgt:
            from .sku_disc import SkuDiscProSubManagerSpDiscProTrgt
            self.__sku_disc_tgt = SkuDiscProSubManagerSpDiscProTrgt(self)
        return self.__sku_disc_tgt

    @property
    def camp(self):
        if not self.__camp:
            from .disc_camp import SpProCampSubManagerTarget
            self.__camp = SpProCampSubManagerTarget(self)
        return self.__camp
    ...
    def fk_mapping_on_disc_trgt(self):
        ...
        # Loop over each SKU
        for sku in skus:
            discovered_keyword_targets = self.filter(target__isnull=True,
            ...
            list_of_key_to_populate = list(discovered_keyword_targets.filter(
                sku_disc_tgt__name__istartswith='b0').values('id', 'target_name'))
    ...
    def associater(self):
        disc_targ = self.filter(target_id_code__isnull=True)
        if not disc_targ.exists():
            return
        df = pd.DataFrame.from_records(disc_targ.values())
        df['target'] = df.apply(
            lambda row: {'target_id_code': row['target_id_code']}, axis=1)
        if not df.empty:
    ...
    def update_spend_matrix(self):
        ...
        targets_df = pd.DataFrame(
            list(self.filter(target__isnull=False).values()))
        # Fetch all spend data at once
        spend_data_df = pd.DataFrame(
            list(AmzAdsSpTgtRpt.objects.filter(target__isnull=False).values()))
        ...
        for _, target in targets_df.iterrows():
            ...
            target_spend_data = spend_data_df[spend_data_df['target_id']
                                              == target['target_id']]
    ...
    def publish_create(self, qs=None):
        if qs is None:
            qs = self.all()
        disc_pro_qs = qs.filter(target__isnull=True, target_id_code__isnull=True,
                                disc_campain__campaign_id_code__isnull=False,
                                disc_campaign__adgroup_id_code__isnull=False
                                ).values('id', 'disc_campaign__campaign_id_code', 'disc_campaign__adgroup_id_code', 'sku_disc_pro__name', 'bid')
        if not disc_pro_qs.exists():    # This makes Early Exit, avoid the overhead of looping through an empty queryset
            return
        df = pd.DataFrame.from_records(disc_pro_qs)
        result_list = []
        ...
        df_response = self.publisher.create_product_targeting_asin(
            df_body)  # API Call
        df_response['created_at'] = timezone.now()
        # Convert NaN values in target_id_code to None
