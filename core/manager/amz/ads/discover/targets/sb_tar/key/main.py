...
class SBDiscKeyTgtManager(BaseManager):
    ...
    def amz_sku(self):
        if not self.__amz_sku:
            from .amz_sku import AmzSkuSubManagerSBDiscKeyTgt
            self.__amz_sku = AmzSkuSubManagerSBDiscKeyTgt(self)
        return self.__amz_sku

    @property
    def sku_disc_tgt(self):
        if not self.__sku_disc_tgt:
            from .sku_disc import AmzSkuDiscKeySubManagerSBDiscKeyTgt
            self.__sku_disc_tgt = AmzSkuDiscKeySubManagerSBDiscKeyTgt(self)
        return self.__sku_disc_tgt

    @property
    def camp(self):
        if not self.__camp:
            from .disc_camp import SBDiscKeyCampSubManagerSBDiscKeyTgt
            self.__camp = SBDiscKeyCampSubManagerSBDiscKeyTgt(self)
        return self.__camp
    ...
    def update_spend_matrix(self):
        ...
        spend_data_df = pd.DataFrame(
            list(AmzAdsSbStRpt.objects.filter(keyword__isnull=False).values()))
