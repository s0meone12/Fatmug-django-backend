...
class SBDiscProTgtManager(BaseManager):
    ...
    def amz_sku(self):
        if not self.__amz_sku:
            from .amz_sku import AmzSkuSubManagerSBDiscProTgt
            self.__amz_sku = AmzSkuSubManagerSBDiscProTgt(self)
        return self.__amz_sku

    @property
    def sku_disc_tgt(self):
        if not self.__sku_disc_tgt:
            from .sku_disc import SkuDiscProSubManagerSbDiscProTrgt
            self.__sku_disc_tgt = SkuDiscProSubManagerSbDiscProTrgt(self)
        return self.__sku_disc_tgt

    @property
    def camp(self):
        if not self.__camp:
            from .disc_camp import SbProCampSubManagerTarget
            self.__camp = SbProCampSubManagerTarget(self)
        return self.__camp
    ...
    def publish_create(self, qs=None):
        ...
        df_response = self.publisher.create_product_target_asin(
            df_body)  # API Call
    ...
    def update_spend_matrix(self):
        ...
        spend_data_df = pd.DataFrame(
            list(AmzAdsSbTgtRpt.objects.filter(target__isnull=False).values()))
